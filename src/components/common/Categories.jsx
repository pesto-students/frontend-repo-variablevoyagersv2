				...
				{CATEGORIES.map((ele) => (
					...
					>
						<img
							src={ele.icon}
							alt={ele.title}
							className={`object-cover w-8 h-8 opacity-100 group-hover:opacity-100 ${selectedTag == ele.tagName ? 'opacity-100' : ''}`}
						/>
						<div
							className={`text-xs whitespace-nowrap font-normal group-hover:font-semibold ${
								ele.tagName === selectedTag ? ' opacity-100 font-semibold ' : ''
							}`}
						>
							{ele.title}
						</div>
