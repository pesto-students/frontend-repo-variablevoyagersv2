...
const OnboardingModal = ({ user, onClose }) => {
    ...
    const registerUser = async (userData) => {
        ...
        setGeneralError('');
        try {
            const { data: { data } } = await axiosInstance.post('/auth', { ...userData, role: selectedRole });
            ...
            if (error.response && error.response.status >= 400 && error.response.status < 500) {
                ...
                setGeneralError(error.message); // Ensure we render only the error message string
            }
        }
    };
    ...
    return (
        <>
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                ...
                <button type="button" onClick={onClose} className="text-gray-400 bg-transparent hover:bg-primary hover:text-white rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center">
                            ...
                            <div className="flex flex-col  md:flex-row justify-between gap-4">
                                <Tooltip content="User can only book">
                                    <button
                                        type="button"
                                        value={selectedRole}
                                        className={`flex flex-col justify-center items-center w-full h-20 rounded-md  transition-colors duration-300 ${selectedRole === 'CUSTOMER'
                                ...
                                <Tooltip content="Owner can list properties">
                                    <button
                                        type="button"
                                        value={selectedRole}
                                        className={`flex flex-col justify-center items-center w-full h-20 rounded-md transition-colors duration-300 ${selectedRole === 'OWNER'
                                            ? 'bg-primary text-white'
                                            : 'bg-gray-50 text-gray-800 hover:bg-gray-100'
                                            }`}
